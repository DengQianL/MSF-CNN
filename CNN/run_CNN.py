...
for i in range(len(trainall)):      ##0-9的数   十份中的第i+1份
    ...
    model.fit(train, trainlabel,
              batch_size=128,  # 每个batch的大小为512
              epochs=55,  # 在全数据集上迭代20次
              )
    model.save('cnn.model4')
    ...
    precision1, recall, thresholds = precision_recall_curve(testlabel, predict_y2)
    d = auc(recall, precision1)    ##auc为pr曲线下的面积
    print("{:.5f} ".format(accuracy)+"{:.5f} ".format(c)+"{:.5f} ".format(d)+"{:.5f} ".format(sensitivity)+"{:.5f} ".format(specificity)+"{:.5f} ".format(MCC)+"{:.5f} ".format(precision))
...
print("precision = {:.5f}".format(np.mean(pre)))
...
plt.plot(recall, precision1, 'b', label='mean PR (area = {0:.5f})'.format(np.mean(pr)))
